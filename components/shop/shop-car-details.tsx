...
import OrderModal from "@/components/shop/order-modal";
...
interface ShopCarDetailsProps {
  carId: string;
  similarCars: any[];
}

export function ShopCarDetails({ carId, similarCars }: ShopCarDetailsProps) {
  ...
  useEffect(() => {
    const fetchCar = async () => {
      try {
        const response = await fetch(`/api/shop/${carId}`);
  ...
  return (
    <div className="mx-4 md:mx-32 py-6 md:py-10 font-primary">
      ...
      <div className="mt-16">
        ...
        <CarCarousel cars={similarCars} title="New Premium Cars" />
      </div>
